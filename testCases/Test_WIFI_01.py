...
class Test_WIFI_01:
    ...
    result = 0
    no_rows = 1
    testcase_name = "test_WIFI_01"
    logger = LogGen.loggen()
    int_SSIDNo = 1
    str_WirelessBand = '2.4GHZ'
    str_WirelessName = 'Test IDCPE 01'
    str_WirelessSecurityMode = 'WPA2-PSK'
    str_WirelessPassword = 'TestIDCPE01'
    list_screenshot = []
    list_deviceinfo = []
    ...
    def test_WIFI_01(self, setup):
        try:
            ...
            self.gui.clickSameSettings()
            self.gui.selectWirelessBand(self.str_WirelessBand)
            self.gui.clickWirelessWifiActiveSlider()
            self.gui.setWirelessWifiNetworkName(self.str_WirelessName)
            self.gui.clickWirelessSecurityLevelMoreSecure()
            self.gui.selectWirelessSecurityMode(self.str_WirelessSecurityMode)
            self.gui.clickWirelessGeneratePasswordAutomatically()
            self.gui.setWirelessPassword(self.str_WirelessPassword)
            self.gui.clickWirelessApply()
            self.gui.clickJioImage()

            x = commonmethod.connectWifiAndroid11(self.str_WirelessName, self.str_WirelessPassword, 'wpa2', r=0)
            if x == 'Connected':
    ...
    def test_WIFI_01_excel(self, setup):
