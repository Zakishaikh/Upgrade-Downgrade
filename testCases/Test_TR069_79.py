...
class Test_TR069_79:
    ...
    def test_TR069_79(self, setup):
        try:
            ...
            self.logger.info("************** Case 1 - Active ****************")
            ...
            self.av.selectManagementServerPeriodicInformEnableNotification('Active')

            status = self.acs.TR069_Sendupdate()

            print('ACS status = ' + status)
            self.logger.info("************** ACS status = " + status + " ****************")
            if status == 'PASS':
                self.list_output.append('FAIL')
            else:
                self.list_output.append('PASS')

            self.logger.info("************** Case 2 - Passive ****************")
            self.lp.ACS_LoginPage()
            ...
            self.av.selectManagementServerPeriodicInformEnableNotification('Passive')
    ...
    def test_TR069_79_excel(self, setup):
