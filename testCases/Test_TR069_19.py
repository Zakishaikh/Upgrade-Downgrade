...
class Test_TR069_19:
        ...
        try:
            ...
            if str(self.int_SSIDNo) not in self.omitssid:
                if SSID == self.ssid_JioCentrum_list[self.int_SSIDNo-1] and status == 'PASS':
                    ...
                    self.logger.info('----------- Trying to Connect SSID ' + str(self.int_SSIDNo) + ' on device ----------- ')
                    x = commonmethod.connectWifiAndroid11(self.ssid_JioCentrum_list[self.int_SSIDNo - 1], list_pass[self.int_SSIDNo - 1], 'wpa2', r=0)
                    if x == 'Connected':
                        self.logger.info('SSID ' + str(self.int_SSIDNo) + ' Name = ' + self.ssid_JioCentrum_list[self.int_SSIDNo - 1])
                    ...
                    self.logger.info('----------- SSID ' + str(self.int_SSIDNo) + ' Update Task Failed on ACS ----------- ')
                    self.list_output.append("FAIL")
            else:
                print("SSID " + str(self.int_SSIDNo) + " is not tested, Since it is in omitssid list " + self.omitssid + "")
    ...
    def test_TR069_19_excel(self, setup):
