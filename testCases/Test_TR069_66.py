...
class Test_TR069_66:
    ...
    def test_TR069_66(self, setup):
        try:
            ...
            status = self.acs.TR069_TaskStatus()
            print('***************** Case 3 Status = ' + status + ' *****************')
            self.logger.info('***************** Case 3 Status = ' + status + ' *****************')
            if status == 'PASS':
                self.list_output.append('PASS')
            else:
                self.list_output.append('FAIL')
    ...
    def test_TR069_66_excel(self, setup):
