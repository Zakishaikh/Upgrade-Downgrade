...
class Test_TR069_86:
    ...
    def test_TR069_86(self, setup):
        try:
            ...
            self.av.clickTableDataRadio_index(2)
            ...
            self.logger.info(
                "************** Updating Wifi Radio 2 AutoChannel Enable Value ****************")
            self.av.setRadioAutoChannelEnable(2, self.int_Enable)
            ...
            str_CurrentChannel = self.jc.getBasicConfigurationCurrentChannel()
            str_bandwidth = self.jc.getBasicConfigurationChannelSpacing()

            if str_mode == 'a/n/ac':
                if '161' in str_CurrentChannel or '165' in str_CurrentChannel:
                    if str_bandwidth == '80MHz':
                        self.list_output.append('PASS')
                    else:
                        self.list_output.append('FAIL')
                else:
    ...
    def test_TR069_86_excel(self, setup):
